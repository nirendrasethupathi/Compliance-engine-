...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; }
        ...
        .info-value { font-size: 10px; margin-left: 4px; }
        .register-table { width: 100%; border-collapse: collapse; font-size: 9px; margin-bottom: 10px; table-layout: fixed; }
        .register-table th, .register-table td { border: 1px solid black; padding: 3px 4px; text-align: center; vertical-align: middle; word-wrap: break-word; line-height: 1.2; }
    ...
    <table class="register-table">
        ...
        <tbody>
            @if(isset($rows) && count($rows) > 0)
                ...
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td style="text-align:left">{{ $row['contractor_name'] ?? '' }}@if(!empty($row['contractor_address']))<br>{{ $row['contractor_address'] }}@endif</td>
                ...
                @endforeach
            @else
                <tr><td colspan="7" style="text-align:center; padding:6px;">No records found</td></tr>
            @endif
        </tbody>
    </table>
